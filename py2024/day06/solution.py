...
class Solver(BaseSolver):
    ...
    def walk(
        self, grid: Grid, pos: Point, dir: Direction, new_obstacle: Point | None = None
    ) -> "Solver.Result":
        seen = set()
        seen_with_dir = set()
        ...
        while (pos, dir) not in seen_with_dir:
            ...
            match "#" if neighbor_pos == new_obstacle else grid.get(neighbor_pos):
                case None:
                    return self.Result(seen, possible_obstacles, False)
                case "#":
                    dir = dir.clockwise
                case _:
                    ...
                    pos = new_pos
        return self.Result(seen, possible_obstacles, True)
